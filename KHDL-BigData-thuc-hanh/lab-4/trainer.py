...
import os
transforms_file = os.path.abspath("lab-4/transforms/transforms.py")
print(transforms_file)
class SparkConfig:
    appName = "CIFAR"
    receivers = '*'
    host = "local"
...
class Trainer:
    ...
    def __train__(self, timestamp, rdd: pyspark.RDD) -> DataFrame:
        ...
        if not rdd.isEmpty():
            # Inspect the type of the image field
            sample = rdd.take(1)[0]
            print(f"Image type: {type(sample[0])}, Image shape: {np.array(sample[0]).shape if isinstance(sample[0], (list, np.ndarray)) else 'N/A'}")
    ...
    def __predict__(self, rdd: pyspark.RDD) -> DataFrame:     
        if not rdd.isEmpty():
            ...
            self.cm += cm
            self.test_accuracy += accuracy/self.total_batches
            self.test_loss += loss/self.total_batches
            self.test_precision += precision/self.total_batches
            self.test_recall += recall/self.total_batches
            self.test_f1 += f1/self.total_batches
